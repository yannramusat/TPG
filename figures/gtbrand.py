...
class FigureGTBRandomConflicts(Figure):
    ...
    def compute(self):
        # execute the scenario GTB with random generation of conflict
        from scenarios.gtb import GUSToBIOSQLRandomConflicts
        for p in self.probs:
            for i in self.x:
                scenario = GUSToBIOSQLRandomConflicts(self.prefix, size=i, prob_conflict = p)
    ...
    def plot(self):
        ...
        plt.savefig("outfigs/FigureExhaustiveRandomGTB.png")

    def print_cmd(self):
        print("## Figure for evaluating the impact of the frequency of conflicts | GUSToBIOSQL scenario")
