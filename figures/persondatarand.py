...
class FigurePersonDataRandomConflicts(Figure):
    ...
    def compute(self):
        # execute the scenario PersonData with random generation of conflict
        from scenarios.persondatas1 import PersonDataScenarioS1RandomConflicts
        for p in self.probs:
            for i in self.x:
                scenario = PersonDataScenarioS1RandomConflicts(self.prefix, size=i, prob_conflict = p)
    ...
    def print_cmd(self):
        print("## Figure for evaluating the impact of the frequency of conflicts | PersonData scenario")
