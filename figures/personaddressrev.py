...
class FigureComparisonIndexesPersonAddressRev(Figure):
    ...
    def compute(self):
        # execute the alternative implementation of the scenario PersonAddress (Rev) with Separate indexes
        from scenarios.personaddressrev import PersonAddressScenarioSeparateIndexesRev
        for i in self.x:
            scenario = PersonAddressScenarioSeparateIndexesRev(self.prefix, size=i)
        ...
        # execute the plain implementation of the scenario PersonAddress (Rev)
        from scenarios.personaddressrev import PersonAddressScenarioPlainRev
        for i in self.x:
            scenario = PersonAddressScenarioPlainRev(self.prefix, size=i)
        ...
        # execute the alternative implementation with Conflict Detection of the scenario PersonAddress (Rev) based on Separate index
        from scenarios.personaddressrev import PersonAddressScenarioCDoverSIRev
        for i in self.x:
            scenario = PersonAddressScenarioCDoverSIRev(self.prefix, size=i)
        ...
        # execute the alternative implementation with Conflict Detection of the scenario PersonAddress (Rev) based on Plain implementation
        from scenarios.personaddressrev import PersonAddressScenarioCDoverPlainRev
        for i in self.x:
            scenario = PersonAddressScenarioCDoverPlainRev(self.prefix, size=i)
    ...
    def print_cmd(self):
        print("## Figure for exhaustive comparison of indexes | PersonAddress (Rev) scenario")
...
class FigureComparisonAlternativeApproachesPersonAddressRev(Figure):
    def __init__(self, app, prefix, values=[], nbLaunches=1, showStats=True):
        super().__init__(app, prefix, values, nbLaunches, showStats)
        # results
    ...
    def plot(self):
        ...
        import matplotlib.pyplot as plt
        import numpy as np
        # Figure for comparing alternative implementations | PersonAddress (Rev) scenario
        ...
        ax.set_title("Comparison of alternative implementations | PersonAddress (Rev) scenario")
