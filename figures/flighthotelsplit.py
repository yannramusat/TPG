...
class FigureComparisonFlightHotelSplit(Figure):
    ...
    def plot(self):
        ...
        ax.plot(self.x, self.results_Sep_long_split, label="Separate indexes alternative (Split)")
        ax.plot(self.x, self.results_Plain_long_split, label="Plain implementation (Split)")
        ax.plot(self.x, self.results_Conflicting_long_split, label="Variant with conflicts (Split)")
